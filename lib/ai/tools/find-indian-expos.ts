...
export const findIndianExpos = tool({
  description: 'Search for specific upcoming trade shows, exhibitions, and expos in India. Returns 5 actual expo events with names, dates, venues, contact details, and descriptions.',
  inputSchema: z.object({
    productType: z.string().describe('Type of products the seller wants to showcase (e.g., textiles, electronics, food, handicrafts)'),
    location: z.string().optional().describe('Preferred location/city in India for the expo'),
    timeframe: z.string().optional().describe('When the seller wants to participate (e.g., next 3 months, 2024, Q1 2025)'),
  }),
  execute: async ({ productType, location = '', timeframe = '' }) => {
    ...
    return {
      ...
      instructions: 'After this tool call, perform web searches to find 5 specific expo events with all the required details including names, dates, venues, organizer contacts, and descriptions.',
    };
  },
});